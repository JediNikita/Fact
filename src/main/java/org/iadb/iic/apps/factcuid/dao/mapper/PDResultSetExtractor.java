...
public class PDResultSetExtractor implements ResultSetExtractor {
	...
	public CompanyFinancials extractData(ResultSet rs) throws SQLException, DataAccessException {
		...
		while(rs.next()) {
			...
			cf.setApprovedDateTime(rs.getTimestamp("APPROVED_DATETIME")); //APPROVED_DATETIME
			cf.setApprovedPD(rs.getString(""));
			cf.setCurrentPercForPDGrade(rs.getBigDecimal("CURRENT_PERC_FOR_PD_GRADE"));
			cf.setExpirationDate(rs.getTimestamp("EXPIRATION_DATE"));
